	...
	<head>
		...
		<script type="text/javascript">
			...
			let deleteFn = function(id){
				window.location.href = "deleteCar?id=" + id; // id 是商品id
			};
			/** id: 物品的ID, num: 购买的数量 , type : 是加一还是减一  storage 商品库存*/
			let addFun = function(id, num, type,storage){
				// 购买的数量
				let buyNum = parseInt(num);
				if (buyNum === 1 && type === 2){
					return;
				}
			...
			let blurFn = function(obj, id, num,storage){
				if (isNaN(obj.value)){
					obj.value = num;
			...
			function submitOrder(){
				let boxs = document.getElementsByName("box"); // 购物车的id
				//保存传递到后台的数据
	        ...
	        <!-- 购物车不为空 <input  type="checkbox"  name="${shopCar.article.id}">-->
	        			...
	        			<c:forEach items="${shopCars}"  var="shopCar">
							<div class="shoppingcart_promotions_01">
								<ul class="shoppingcart_list">
									...
									<li class="row00">
										<a name="productpic" href="doItem?id=${shopCar.article.id}" title="${shopCar.article.title}">
									...
									<li class="row11">
										<p>
											<span class="name">
												<a name="product"  href="doItem?id=${shopCar.article.id}" title="${shopCar.article.title}">${shopCar.article.title}</a>
											</span>
										</p>
									</li>
									<li class="row22">0</li>
		        ...
		        <div align="right" id="div_total" style="display: block;clear:both;">
		            ...
		            <p>
		                ...
		                <form action="${ctx}/doOrder?step=1" method="post" id="form">
				            <td><a href="${ctx}/toIndex" name="goon" class="goon"><img alt="" src="images/shop.jpg"/></a></td>
