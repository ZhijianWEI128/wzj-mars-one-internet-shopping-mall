	...
	<head>
		...
		<script type="text/javascript">
            ...
            function reloadIndex(val){
           		...
           		window.location = "${pageContext.request.contextPath}/toIndex?typecode="+val+"&keyword="+keyword;
            }
		</script>
	</head>
<body>
	...
	<div style="positon: relative; width: 960px;margin: 0px auto;">
		...
		<div  id="booksort" style="float:left;width:210px;">
			...
			<div class="mc">
					<c:forEach items="${firstArticleTypes}" var="firstArticleType" >
					 <div class="item"><h3><b>&gt;</b><a href="/toIndex?typecode=${firstArticleType.code}">·${firstArticleType.name}</a></h3></div>
					</c:forEach>
			</div>
		</div>
		<!-- 右边对应物品列表 -->
		<div style="float:left;width:750px;text-align:center;">
			<div>
				<form action="toIndex" method="post" name="search" >
			  		物品类型：
			  		<select name="typecode" id="typecode"  onchange="reloadIndex(this.value)">
				  		<option value="${firstArticleType.code}">---------全部---------</option>
<%--							<option value="">全部</option>--%>
				  			<c:forEach  items="${allSecondArticleTypes}"  var="secondArticleType">
				  				<option value="${secondArticleType.code}">----${secondArticleType.name}</option>
				  			</c:forEach>
			  		</select>
				    <input name="keyword" id="keyword" type="text" value="${keyword}" size="50"/>
				    <button>搜索</button>
			  	</form>
			</div>
			<!-- 显示所有商品 -->
				...
				<div class="sales-queue" id="tabs-1" style="background-color:white;margin-top:-25px;">
				    <ul class="goods-queue3">
						<c:forEach  items="${articles}"  var="article">
							<li id="selbgc11">
							<dl class="item-des">
							  <dt><a href="doItem?id=${article.id}" title="${article.title}" target="_self">
							  <img src="images/article/${article.image}" width="132" height="96" /></a></dt>
							  <dd><s>¥:<fmt:formatNumber  value="${article.price}"  pattern="0.00"></fmt:formatNumber>  </s>
							  <strong>¥:<fmt:formatNumber  value="${article.price* article.discount}"  pattern="0.00"></fmt:formatNumber> </strong></dd>
							  <dd><h2><a href="doItem?id=${article.id}" title="${article.title}" target="_self">${article.title}</a></h2></dd>
							</dl>
						  </li>
						</c:forEach>
					</ul>
